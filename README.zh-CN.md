...
- 当前 skill/工具集无法满足某个交付质量时，先记录 `capability_gaps`，再考虑采用新工具。优先走 `local_only` 或 `project_local` 试验；通用 kit 变更走 `omykit_candidate_branch`；没有来源、license、安装/运行、真实输出和回滚证据，不要把新的外部 skill 直接提交进主线。`hugohe3/ppt-master` 是原生可编辑 PPTX 缺口的已跟踪候选，不是默认依赖。
