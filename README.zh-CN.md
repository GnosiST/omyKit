...
- **Scorecard 验票：**先检查真实 handoff、入口决策、验证证据、语言一致性、skill 使用、用量记录、推荐模型和实际模型记录，再相信完成声明。
