...
- **Scorecard 验票：**先检查真实 handoff、验证证据、语言一致性、skill 使用、用量记录和模型档位策略，再相信完成声明。
...
该命令会写入 `.omykit/workflows/<workflow-id>/board.json` 和 `board.html`。新的追踪型 workflow 可以选择 `change.standard`、`bugfix.standard`、`frontend-ui.strict` 等可复用模板；未指定时使用 `change.standard`。看板语言默认跟随 workflow 语言，也可以用 `--lang zh-CN` 显式覆盖。handoff 提供记录时，看板还会展示每个节点和 worker 实际使用的 skill。这是本地静态视图，不是实时服务。
