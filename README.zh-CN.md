...
- **持久任务图：**长任务、可续跑任务和多节点工作可使用本地 C-lite controller。
...
长任务或 Strict 工作可以把任务图持久化到 `.omykit/workflows/<workflow-id>/`，并用 `scripts/omykit-workflow.mjs` 校验 handoff、查看 ready 节点、记录 blockers，并支持 compact 后续跑。
