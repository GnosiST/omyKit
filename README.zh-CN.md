...
- **子智能体派发计划：**主 Codex 对话保持 orchestrator-observer，只从就绪节点派发有边界的 worker，并且只在子智能体运行时支持时使用模型 override。
...
`开始执行` 表示 Codex 应该创建或续跑 workflow、启动就绪节点、执行节点真实工作、写 handoff，并持续推进到 delivery 通过或遇到真实阻塞。只有想先拿骨架和手动续跑命令时，才用 `只创建工作流`。Codex 会在内部运行 controller，并返回生成路径。项目终端中的手动 fallback：

```bash
node scripts/omykit-workflow.mjs board --open --lang zh-CN
```

该命令会写入 `.omykit/workflows/<workflow-id>/board.json` 和 `board.html`。新的追踪型 workflow 可以选择 `change.standard`、`bugfix.standard`、`frontend-ui.strict` 等可复用模板；未指定时使用 `change.standard`。看板语言默认跟随 workflow 语言，也可以用 `--lang zh-CN` 显式覆盖。handoff 提供记录时，看板还会展示每个节点和 worker 实际使用的 skill、推荐模型和实际模型记录。这是本地静态视图，不是实时服务。
...
长任务或 Strict 工作可以把任务图持久化到 `.omykit/workflows/<workflow-id>/`，并用 `scripts/omykit-workflow.mjs` 校验 handoff、查看 ready 节点、记录 blockers、生成静态协作看板，并支持 compact 后续跑。
...
- 创建追踪型 workflow 不等于任务完成；长任务要继续按 `start -> work -> handoff -> complete/reject/block/unblock -> next/resume` 循环推进，直到 delivery 通过或记录真实阻塞。
- 多智能体工作先用 `dispatch-plan`；主对话保持当前模型作为 orchestrator-observer，只在运行时支持时把模型 override 传给子智能体。
...
5. 运行 `./scripts/install-global.sh` 更新全局 Codex skill 副本和 controller 文件。
