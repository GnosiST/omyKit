...
- **原创文档边界：**可以借鉴外部 skill 的协作方式，但不把第三方内容直接复制进仓库。

## 快速开始

安装全局 Codex skills 和 prompt：

```bash
./scripts/install-global.sh
```

打开新的 Codex 线程让 skill 列表刷新，然后使用：
...
| `docs/workflow/` | 设置、路由、上下文预算、运行时准备、版本管理、外部协作和交付门禁文档。 |
...
2. 运行 `./scripts/install-global.sh` 更新全局 Codex skill 副本。
3. 检查 `${CODEX_HOME:-$HOME/.codex}/omykit/install-manifest`。
4. 检查 `git diff --check`。
5. 本地与全局副本验证通过后再提交和推送。
