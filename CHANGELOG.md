...
- Fixed controller handoff projection after independent E2E testing: `complete`/`reject` now canonicalize external handoff files into workflow-local `handoffs/`, object-shaped output/evidence records no longer crash board or scorecard projection, terminal `downstream_context.target_nodes` can be empty, `scorecard --json` emits machine-readable JSON, and tests cover the regressions.
