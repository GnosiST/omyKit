...
- Added regression coverage for cross-workflow isolation so misplaced handoffs or assignments from a similar workflow fail `validate` and `board`, and active-workflow `resume` stays pinned to the selected workflow.
- Clarified validation commands so controller tests inherit the same `PYTHON` runtime when local `python3` lacks `PyYAML`.
