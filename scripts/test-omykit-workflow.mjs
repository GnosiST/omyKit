...
assert.equal(deckTask.workstream, "deck-proposal");
assert.ok(deckTask.suggested_write_scope.includes("presentations/**"));
const taskList = JSON.parse(run(["tasks", "list", "--json"], tmpTasks));
assert.equal(taskList.tasks.length, 4);
assert.equal(taskList.summary.total, 4);
...
assert.equal(taskDoctor.project.task_inbox.total, 4);
