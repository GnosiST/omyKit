...
- **Scorecard audit:** check real handoffs, intake decisions, delivery evolution reviews, verification evidence, communication audits, language consistency, skill usage, usage records, model recommendations, and actual model records before trusting completion claims.
...
- **Global collaboration audit:** when multi-agent work is active, an independent `global-auditor` records `communication_audit` evidence before integration so worker agreement, contradictions, scope drift, and evidence gaps are visible.
...
The board command produces `board.json` for machine-readable projection and `board.html` for browser review. It shows the selected template, current orchestration topology, scorecard results, task inbox, merge-gate decisions, workstreams, conflict-arbiter signals, intake decisions, workflow evolution candidates, global communication audits, delivery knowledge sync review, a clickable task tracker with actual node work items, changed-file summaries, recorded skill usage, verification results, evidence availability, downstream handoff context, generated handoff packets, command-run recovery records, agent activity, Agent Roster assignments, runtime policy blockers, recommended model tiers, recommended concrete models, actual model records, usage-observation status, token and context coverage, task contract size, context source breakdowns, per-node timing, ETA estimates, project snapshot, dependency/reject flow, worker lanes, blockers, decisions, retries, events, and generated improvement actions without introducing a server or database. Provider token totals only aggregate recorded evidence; unavailable runtime metrics are shown separately from missing records and are never guessed. Context totals include recorded handoff/agent context plus deterministic controller measurements from generated context packs or node contracts, clearly marked by source.
...
- When multi-agent work produces multiple worker/assignment records, require an independent global communication audit before integration. The audit must record reviewed agents, reviewed handoffs, evidence independence, contradictions, scope drift, findings, and whether any worker agreement lacks evidence.
...
- Use `deck.proposal` for PPT, presentation, slide, pitch-deck, or proposal-deck work; the controller classifies the task as `create` (new deck), `remake` (rebuild an existing deck), or `modify` (targeted edits that preserve the original template). Intake should offer direction options, record the selected production surface and template strategy, and keep same-lane deck skill fallback auditable instead of stacking every PPT skill.
...
- When the current skill/tool set cannot satisfy a deliverable quality, record `capability_gaps` before adopting new tools. Use `local_only` or `project_local` trials first; use `omykit_candidate_branch` for generic kit changes; do not commit a new external skill into mainline without source, license, install/run, real-output, and rollback evidence. `hugohe3/ppt-master`, `op7418/guizang-ppt-skill`, `zarazhangrui/beautiful-html-templates`, and `irenerachel/visual-style-ppt-skill` are PPT/deck references with different trust and license boundaries; they are not vendored dependencies.
