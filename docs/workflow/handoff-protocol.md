...
Token, context, and model records must be source-aware. If a `token_usage` or `context_usage` object is present, `source` is required. Record exact provider/tool-reported token usage when available; otherwise use `usage_observation.token_status=unavailable` or omit the token record. Do not invent Codex Desktop or chat token counts when the environment does not expose them. Context size is separate from provider token usage: controller-generated context packs include deterministic serialized-size estimates, and workers may add exact source/context records when their runtime exposes them. Use `model_tier` as a supplier-independent policy (`fast`, `standard`, `frontier`); record the actual provider/model only as observed execution metadata through `model`, `model_provider`, `token_usage.model`, `agent_activity[].model`, `agent_activity[].model_provider`, or `agent_activity[].token_usage.model`. Codex Desktop worker creation may support model overrides when current tool policy allows it, so a node handoff should record the recommended model and actual model when execution exposes it. If runtime policy or metadata hides the actual model or token counts, add node-level `usage_observation` with `model_status` or `token_status` set to `unavailable` and include the corresponding unavailable reason. The board treats unavailable runtime data separately from missing records.
...
- Treat compaction as lossy. Preserve future-critical facts in `downstream_context` and evidence/source paths, and rely on generated context packs rather than full conversation history for worker handoff or resume.
