...
| PPT/deck 生成、重制或修改 | 使用 `deck.proposal` 的 `codex-change-workflow` | `codex-context-budget`，记录 `deck_variant`，优先 bundled `presentations`/Canva/项目模板；可选 deck specialist 通过 `skill_decisions` 和 `capability_gaps` 记录，再进入 `codex-delivery-gate`。 |
