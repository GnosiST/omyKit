...
- 入口阶段先展示目标、所选路由、执行形态或 controller 模板，以及影响交付的关键假设，再进入实现。
- 只有安全假设会带来风险，且答案会改变交付物、目标项目、成功标准、风险边界、运行时约束、workflow 模式、controller 需求或模板选择时，才问 1-3 个问题。提问时可以给建议选项，但必须允许用户自定义答案。
...
Controller 把状态保存到 `.omykit/workflows/<workflow-id>/`，校验 handoff 文件，把 skill、agent、模型和用量证据投影到看板，推荐 ready 节点和合适模型，并降低 compact 后续跑成本。它不调用模型，不替代 Codex，也不会让 Lite 任务默认变重。
