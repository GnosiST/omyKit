...
`assign` appends the real runtime assignment to `assignments.jsonl`: node, agent id, role, execution surface, thread id, worktree, model tier, write scope, context pack, handoff path, runtime policy blocker, and status. It is a runtime roster, not a template field. The board projects the Agent Roster and runtime policy blockers, and scorecards warn when assignments lack readable handoffs or active agents have overlapping write scopes.

When multiple workers or assignments are active, integration should not depend only on the main thread summarizing worker chat. A `global-auditor` node, or an equivalent terminal handoff, records `communication_audit`: reviewed agents, reviewed handoffs, reviewed assignments when available, evidence independence, contradiction status, scope-drift status, findings, and whether any worker agreement lacked evidence. `mission.orchestration` includes this node before integration; other templates surface recommended scorecard warnings only when multi-agent evidence actually appears.
...
The board also renders the selected workflow template and scorecard audit. Scorecards inspect recorded evidence; they do not trust natural-language completion claims by themselves. Passed intake nodes must record `intake_decision`, including route, workflow shape, assumptions, execution options, selected option, confirmation status, and custom-answer policy. If a downstream reject returns intake to `ready`, the scorecard still treats the previous readable passed intake handoff as historical evidence instead of forcing users to recreate the original decision record. Passed global-auditor nodes must record `communication_audit`; open or blocking audit findings become scorecard failures or recommended warnings depending on the selected template. Passed delivery nodes must record `evolution_candidates`; an empty array means the delivery was reviewed and no reusable lesson was found. They must also record `knowledge_sync` as `completed`, `not_needed`, or `deferred` with a reason, so docs/AGENTS/memory cleanup is not forgotten during handoff. Generic candidates are surfaced as improvement actions for `codex-workflow-evolution` review. Failed scorecard checks become improvement actions with affected node links when possible. Skill-usage, skill-selection, communication-audit on non-mission templates, and actual-model checks are recommended warnings: they expose missing records without forcing fake skill, same-lane choice, global-audit, or model records where they do not apply.
