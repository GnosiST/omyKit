...
The board also renders the selected workflow template and scorecard audit. Scorecards inspect recorded evidence; they do not trust natural-language completion claims by themselves. Passed intake nodes must record `intake_decision`, including route, workflow shape, assumptions, and custom-answer policy. Failed scorecard checks become improvement actions with affected node links when possible. Skill-usage and actual-model checks are recommended warnings: they expose missing records without forcing fake skill or model records where the runtime did not expose them.
