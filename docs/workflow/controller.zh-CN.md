...
2. 编排计划决定下一步用主线程、同 turn 子智能体、后台线程还是 worktree；用户不应该手动选择这个底层执行面。
3. 如果节点要派给 worker 或 compact 后续跑，Codex 内部先生成 `context-pack <node-id>`。
4. Codex 在当前项目里执行该节点的真实工作，或把有边界的 context pack 交给 worker。
5. 真实 worker/thread/worktree 存在后，Codex 用 `assign` 记录分工。
6. 对 dev server、测试 watcher、长构建或截图服务等后台命令，用 `record-run` 记录命令、日志、pid 和续接命令。
7. Codex 写结构化 handoff，记录工作项、证据、`downstream_context`、可用的 skill/model/用量记录，以及 delivery 节点的 `evolution_candidates` 和 `knowledge_sync`。
8. Codex 运行 `complete`、`reject`、`block`，或在已记录的阻塞解决后运行 `unblock`。
9. 重复循环，直到 delivery 通过、真实阻塞需要用户处理，或用户明确要求停止。
