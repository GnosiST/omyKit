...
| 可复用模板 | `change.standard`、`bugfix.standard`、`frontend-ui.strict` 和 `mission.orchestration`，并把拓扑、agent、模型、运行环境、安全限位、scorecard 分层。 | 已覆盖普通变更、缺陷修复、严格 UI 和复杂需求编排；后续只有拓扑明显不同才新增模板。 |
