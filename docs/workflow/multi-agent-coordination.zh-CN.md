...
| 子智能体并行 | `orchestrate` 给出自动执行模式、下一批动作、worker profile、推荐模型和 context pack 策略。 | Controller 负责决策计划；实际启动 worker 仍由 Codex runtime 执行。 |
...
2. 真实 worker 存在后，`assign` 命令写入 `.omykit/workflows/<id>/assignments.jsonl`，记录 `thread_id`、worktree、worker、模型、scope、状态、context pack 和 handoff 路径。
3. 看板新增 Agent 通讯录，显示每个 agent 的角色、执行面、线程/worktree、节点和状态。
4. Scorecard 新增 assignment handoff 覆盖和写入范围冲突检查。
5. Compact recovery 顺序加入 `orchestration-plan.json` 和 `assignments.jsonl`，让主控恢复时能先读计划和通讯录再决定续接哪个 worker。
6. Task Inbox 和 Merge Gate 记录多次用户 brief，把同源任务并入当前 workflow 或链接到历史 workflow，并在看板中显示工作流组和冲突仲裁项。
