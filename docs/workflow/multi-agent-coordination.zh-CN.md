...
| 多 thread / worktree agent | 当前没有内建通讯录、thread id 记录或 worktree 后端。 | 主要缺口。 |
...
| 运行层 | `state.json` / `ledger.jsonl` / future `assignments.jsonl` | 实际分配给哪个 Codex thread、subagent、worktree、模型、开始/结束时间、证据路径。 |
...
1. 文档与 skill 层先支持 thread-native 语义：在 `omykit` 和 controller docs 中区分 `subagent`、`background_thread`、`thread_worktree`。
2. controller 增加 `dispatch-plan --surface subagent|thread|worktree|auto`，让派发计划能输出可创建 thread 的任务说明。
3. 增加运行时分配账本，例如 `.omykit/workflows/<id>/assignments.jsonl`，记录 `thread_id`、worktree、worker、模型、scope、状态和 handoff 路径。
4. 看板新增“Agent 通讯录 / Thread Map”：显示每个 agent 的角色、执行面、线程状态、工作范围、上下游关系、最后事件和需要用户介入的节点。
5. scorecard 增加 thread-aware 审计：同一文件是否被多个 worker 写、每个 background thread 是否有 handoff、thread summary 是否进入主 workflow。
6. 对写操作制定保守策略：默认不要两个 background threads 改同一文件；需要并行写时必须 disjoint write scope 或 worktree 隔离。
7. 引入更强 resume：主控 compact 后先读 active workflow、assignments、context pack、thread summaries，再决定继续哪个 thread 或 handoff 到本地。
...
下一轮可实现一个最小但正式的 thread-native 协作版本：

1. 新增 `dispatch-plan --surface auto`，输出每个 ready node 推荐 `main-thread`、`subagent`、`background_thread` 或 `thread_worktree`。
2. 新增 `assign` / `assignment-record` 命令，写入运行时通讯录。
3. 扩展 board，新增“Agent 通讯录”和“Thread Map”。
4. 更新 scorecard，检查 background worker 是否提交结构化 handoff。
5. 用 omyKit 自身做 dogfood：只把文档和测试拆给独立 thread，避免多个 thread 同时改同一核心脚本。
