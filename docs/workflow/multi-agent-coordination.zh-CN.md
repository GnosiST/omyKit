...
| 多 thread / worktree agent | 已有 `dispatch-plan --surface`、`assign`、`assignments.jsonl`、Agent 通讯录、handoff 覆盖、写入范围 scorecard 和 N:1 汇聚跟踪。 | 已具备记录、审计和 join 可见性；controller 仍不直接创建 thread/worktree。 |
...
结论：当前 omyKit 已经从“skill 说明集合”升级为“Skill + 轻量 controller + YAML 模板 + scorecard + 看板”的工作流套件，符合大部分核心需求。controller 现在会在 `orchestration-plan.json` 中明确 1:1、1:N、N:1 协作形态；剩余最大短板是缺少一个运行时派发助手，去消费这些契约，并在当前 Codex 工具面允许时自动创建 thread/worktree。
...
4. 真实 worker 存在后，`assign` 命令写入 `.omykit/workflows/<id>/assignments.jsonl`，记录 `thread_id`、worktree、worker、模型、scope、状态、context pack 和 handoff 路径。
5. 看板新增 Agent 通讯录，显示每个 agent 的角色、执行面、线程/worktree、节点和状态。
6. Scorecard 新增 assignment handoff 覆盖和写入范围冲突检查。
