...
| 多 thread / worktree agent | 已有 `dispatch-plan --surface`、`assign`、`assignments.jsonl`、Agent 通讯录、运行时策略阻塞、handoff 覆盖、写入范围 scorecard 和 N:1 汇聚跟踪。 | 已具备记录、审计、策略阻塞可见性和 join 可见性；controller 仍不直接创建 thread/worktree。 |
...
结论：当前 omyKit 已经从“skill 说明集合”升级为“Skill + 轻量 controller + YAML 模板 + scorecard + 看板”的工作流套件，符合大部分核心需求。controller 现在会在 `orchestration-plan.json` 中明确 1:1、1:N、N:1 协作形态，并在 Codex 无法创建指定 worker 时记录运行时策略阻塞；剩余最大短板是缺少一个运行时派发助手，去消费这些契约，并在当前 Codex 工具面允许时自动创建 thread/worktree。
...
7. Compact recovery 顺序加入 `orchestration-plan.json` 和 `assignments.jsonl`，让主控恢复时能先读计划和通讯录再决定续接哪个 worker。
8. Task Inbox 和 Merge Gate 记录多次用户 brief，把同源任务并入当前 workflow 或链接到历史 workflow，并在看板中显示工作流组和冲突仲裁项。
