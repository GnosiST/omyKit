...
| 多次同源任务处理 | 项目级 Task Inbox 记录每次 brief，Merge Gate 自动判定 `merge_current`、`linked_follow_up` 或 `new_workflow`，并把写入范围重叠投影到冲突仲裁。 | 已落地基础能力；合并判断仍是启发式，需要真实项目反馈继续进化。 |
| 长任务不在创建 workflow 后停下 | skill、docs 和 controller 状态输出要求 `resume/orchestrate -> 启动或派发 -> work -> handoff -> complete/reject/block` 循环。 | 规则和编排产物已补齐；真实执行仍依赖当前 Codex turn 的持续性。 |
| 可复用模板 | `change.standard`、`bugfix.standard`、`frontend-ui.strict`、`deck.proposal` 和 `mission.orchestration`，并把拓扑、agent、模型、运行环境、安全限位、scorecard 分层。 | 已覆盖普通变更、缺陷修复、严格 UI、PPT 生成/重制/修改和复杂需求编排；后续只有拓扑明显不同才新增模板。 |
| 结构化 handoff | schema、controller 校验、`downstream_context`、`communication_audit`、work items、evidence、skills、model、token/context、timing。 | 已比较完整。 |
| 子智能体并行 | `orchestrate` 输出 `collaboration_topology`、下一批动作、`dispatch_batch_id`、worker profile、推荐模型和 context pack 策略。 | 1:1 和 1:N 派发决策已明确；实际启动 worker 仍由 Codex runtime 执行。 |
| 多 thread / worktree agent | 已有 `dispatch-plan --surface`、`assign`、`assignments.jsonl`、Agent 通讯录、运行时策略阻塞、handoff 覆盖、写入范围 scorecard、全局协作审计和 N:1 汇聚跟踪。 | 已具备记录、审计、策略阻塞可见性和 join 可见性；controller 仍不直接创建 thread/worktree。 |
| 低上下文续接 | `active-workflow`、`context-pack`、`downstream_context`、`commands/commands.jsonl`。 | 已有基础；还缺 thread-aware resume packet。 |
| 看板 | 展示任务、证据、全局协作审计、skill、模型、token/context、时间、命令、交接包和整改建议。 | 已从通用模板升级为项目任务追踪；还不是实时调度台。 |
...
结论：当前 omyKit 已经从“skill 说明集合”升级为“Skill + 轻量 controller + YAML 模板 + scorecard + 看板”的工作流套件，符合大部分核心需求。controller 现在会在 `orchestration-plan.json` 中明确 1:1、1:N、N:1 协作形态，在 Codex 无法创建指定 worker 时记录运行时策略阻塞，并在多 agent 证据出现时要求或提醒 `global-auditor` 在集成前完成协作审计；剩余最大短板是缺少一个运行时派发助手，去消费这些契约，并在当前 Codex 工具面允许时自动创建 thread/worktree。
...
| 审计层 | `handoff.communication_audit` | 集成前独立审查 worker 一致性、已审查 handoff、证据独立性、矛盾和范围漂移。 |
...
1. `orchestrate` 输出推荐执行模式并写入 `orchestration-plan.json`；`dispatch-plan --surface auto|subagent|thread|worktree|main` 保留为内部原子命令。
2. `orchestrate` 写入 `collaboration_topology`，包含 `one_to_one`、`one_to_many`、`many_to_one` 触发、扇出组、汇聚目标和 `waiting_on` 列表。
3. `dispatch_worker` action 带 `collaboration_pattern`、`dispatch_batch_id`、`depends_on`、`join_policy` 和 `handoff_target`，让 Codex 能创建受限 worker，并正确等待 join 节点。
4. 真实 worker 存在后，或运行时策略阻止 worker 创建时，`assign` 命令写入 `.omykit/workflows/<id>/assignments.jsonl`，记录 `thread_id`、worktree、worker、模型、scope、状态、context pack、handoff 路径和可选 `policy_blocker`。
5. 看板新增 Agent 通讯录，显示每个 agent 的角色、执行面、线程/worktree、节点和状态。
6. Scorecard 新增 assignment handoff 覆盖和写入范围冲突检查；看板和编排投影会把运行时策略阻塞与缺失 handoff 分开展示。
7. `mission.orchestration` 在集成前内置 `global-auditor` 节点；所有 scorecard 都能在出现多 agent 证据时暴露缺失或未解决的 `communication_audit`。
8. Compact recovery 顺序加入 `orchestration-plan.json` 和 `assignments.jsonl`，让主控恢复时能先读计划和通讯录再决定续接哪个 worker。
9. Task Inbox 和 Merge Gate 记录多次用户 brief，把同源任务并入当前 workflow 或链接到历史 workflow，并在看板中显示工作流组和冲突仲裁项。

## 后续优化路线

1. 增加运行时派发助手，消费 `dispatch_worker` action，并在当前 Codex 工具面允许时创建受限 subagent、background thread 或 worktree。
...
5. 把 Task Inbox 的合并启发式升级为可评分规则：按目标、模板、文件范围、页面/模块、截图证据和用户反馈调整同源合并阈值。
6. 增加 thread-aware resume 命令：主控 compact 后先读 active workflow、assignments、context pack、thread summaries，再决定继续哪个 thread 或 handoff 到本地。

## 当前不建议做的事

- 不要让 controller 自动创建无限 background threads；`orchestrate` 可以推荐 worker 执行面，但 thread 创建仍必须由 Codex runtime 在有边界的范围内执行。
