...
The controller stores state under `.omykit/workflows/<workflow-id>/`, validates handoff files, audits intake decisions, projects skill/agent/model/usage evidence to the board, recommends ready nodes and right-sized models, and keeps compact recovery cheap. It does not call models, replace Codex, or make Lite tasks heavy by default.
