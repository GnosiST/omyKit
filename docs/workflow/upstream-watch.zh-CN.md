...
这里的“官方性”指 omyKit 为某项能力跟踪的精确上游或项目主页仓库，不表示仓库所有者对 omyKit 背书。下面快照于 2026-06-23 通过 GitHub repository API 核对；PPT 专项新增来源于 2026-06-24 核对。star 数会变化，只有当来源声誉会实质影响决策时才需要刷新。

低信号来源如果重复 omyKit 的 PM、审美、目录或 meta-UX 能力线，不在这里跟踪。高信号可选参考可以为了来源完整性进入监控，但仍然是按需 opt-in，不会变成默认 route。

| 来源 | 开发者 / 所有者 | 快照 stars | 状态 | omyKit 参考范围 |
| --- | --- | ---: | --- | --- |
...
| [hugohe3/ppt-master](https://github.com/hugohe3/ppt-master) | `hugohe3` / User | 31,026 | 非 fork, 活跃, MIT | 原生可编辑 PPTX 试验和 `deck.proposal` 路由决策的 deck specialist 参考；不复制 skill body、模板、赞助文案、图片、badge 或 branding。 |
| [op7418/guizang-ppt-skill](https://github.com/op7418/guizang-ppt-skill) | `op7418` / User | 18,771 | 非 fork, 活跃, AGPL-3.0 | 可选 HTML deck specialist 参考；AGPL license 需要谨慎，omyKit 不复制 runtime code、模板、图片、badge 或 branding。 |
| [zarazhangrui/beautiful-html-templates](https://github.com/zarazhangrui/beautiful-html-templates) | `zarazhangrui` / User | 3,156 | 非 fork, 活跃, MIT | 外部 HTML slide 模板库参考；omyKit 只链接来源，不缓存或 vendoring 模板文件。 |
| [irenerachel/visual-style-ppt-skill](https://github.com/irenerachel/visual-style-ppt-skill) | `irenerachel` / User | 242 | 非 fork, 活跃, 未检测到 license | 用户要求评估的低信号风格评审候选；不是默认 route，也不是 vendored 依赖。 |
...
这个命令会同时检查 upstream `HEAD` 是否漂移，以及 `upstream-sources.json` 里的来源完整性快照字段是否齐全。在配置的审查周期内发现漂移时会标记为 deferred，避免打断普通任务。
...
`--strict` 只有在已到审查时点且存在漂移时才非 0 退出。`--force-review` 会忽略周期，适合 release 前或任务依赖当前上游行为时使用。`--report <file>` 会写入机器可读漂移报告。
