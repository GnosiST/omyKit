...
- `obra/Superpowers`：执行纪律和验证工作流参考。
- `github/spec-kit`：严格 specification-driven project workflow 参考。
- `Fission-AI/openspec`：proposal-based 变更管理参考。
- `colbymchenry/codegraph`：代码地图和影响分析参考。
- `upstash/context7`：当前库文档查询参考。
- `zhongerxin/Cowart`：视觉画布和空间设计上下文参考。
- `GLips/Figma-Context-MCP`：Figma 设计上下文参考。
- `phuryn/pm-skills`：PM 方法参考。
- `birobirobiro/awesome-shadcn-ui`：shadcn/ui 生态发现。
- `Leonxlnx/taste-skill`：设计品味校准。
- `nextlevelbuilder/ui-ux-pro-max-skill`：UX/设计智能和模式比较参考。
- `anthropics/skills`：官方 `frontend-design` skill 参考。
- `greensock/gsap-skills`：官方 GSAP skills 参考。
- `headroomlabs-ai/headroom`：可选上下文压缩、可逆原文取回和输出整形参考信号。
