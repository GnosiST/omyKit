...
| `deck.proposal` | Strict | 支持从零生成、重制已有 PPT、按原模板局部修改的 PPT/deck 工作，包含方向选项、来源安全 specialist 选择、反馈返工、导出验证和交付证据。 |
| `mission.orchestration` | Strict | 需要需求洞察、任务拆解、工作流路由、执行监听、独立全局协作审计、集成验票和学习沉淀的复杂需求。 |
...
node scripts/omykit-workflow.mjs init "重制已有 PPT：升级季度汇报" --template auto --lang zh-CN
node scripts/omykit-workflow.mjs init "修改 PPT：沿用原模板新增路线图页" --template auto --lang zh-CN
...
- 实际工作项记录
- `deck.proposal` 记录 `deck_variant`：`create`、`remake` 或 `modify`
- 通过且交给下游节点时的下游上下文记录
- 变更文件摘要
- 验证记录和证据路径
- 多智能体工作的协作审计，包括证据独立性、矛盾、范围漂移和未解决发现
...
- 当图拓扑确实不同，新增模板。
- 交付物是 PPT、presentation、slide deck、pitch deck 或提案 deck 时使用 `deck.proposal`。该模板用 `deck_variant` 区分从零生成、重制已有 PPT、按原模板局部修改三类任务，并把源材料/模板分析、故事线、方向、生产、美化、验证、用户反馈返工和交付拆开，让 bundled presentation 工具、Canva、PPT Master、Guizang 和外部模板参考按能力协作，而不是抢同一份工作。
- 当任务需要协调多个 workflow 或 worker 分道时使用 `mission.orchestration`。该模板在集成前包含 `global-auditor` 节点，用 handoff 证据核对多 agent 一致性，而不是只相信 worker 摘要。
